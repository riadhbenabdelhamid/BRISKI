    ...
    if (PIPE_STAGE == 1) begin : registered_reservation_set
        ...
        always_ff @(posedge clk) begin
            if (reset) begin
                ...
                if (i_load_reserved_op) reserved_valid <= 1'b1;

                if ((i_store_cond_op && hart_match) || (i_store_op && address_match)) reserved_valid <= 1'b0;
        ...
        always_latch begin
            if (reset) begin
                ...
                if (reserved_valid) o_sc_success = address_match && i_store_cond_op && hart_match;
