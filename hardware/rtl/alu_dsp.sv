...
module alu_dsp #(
  ...
  //
  // DSP58 specific signals
  logic [           33:0] Ax;  // 34 bits
  logic [           23:0] Bx;  // 24 bits
  logic [           57:0] Cx;  // 58 bits
  //logic [26:0] D;            // 27 bits
  logic [           57:0] Px;  // 58 bits
  //logic [4:0]  INMODE;       // 5 bits
  //logic [8:0]  OPMODE;       // 9 bits
  //logic [3:0]  ALUMODE;      // 4 bits
  ...
  if (PIPE_STAGE2 == 1) begin : third_stage_registered
    //-------------------------------------------------
    always_ff @(posedge clk) o_result <= result_sll | result_dsp | result_srl_sra;

  end else begin : third_stage_not_registered
    //-------------------------------------------------
    always_comb o_result = result_sll | result_dsp | result_srl_sra;
...
`ifdef FPGA_FAMILY_VERSAL
    ...
    //   DSP inputs
    //-----------------------------------------------------------------------
    assign Ax[33:8] = {26{i_op2[31]}};  // sign extend
    assign Ax[7:0] = i_op2[31:24];
    assign Bx = i_op2[23:0];

    // C <= (C'high downto 32=>'0',i_op1(31 downto 0); -- unsigned
    assign Cx[57:32] = {26{i_op1[31]}};  // sign extend
    assign Cx[31:0] = i_op1[31:0];
    assign D = 27'b0;
    ...
    assign result_dsp = Px[31:0];

`else  //Ultrascale, Ultrascale+ and 7SERIES
    //-----------------------------------------------------------------------
    //   DSP inputs
    //-----------------------------------------------------------------------
...
`ifdef FPGA_FAMILY_VERSAL
    ...
    DSP58 #(
        ...
        .P             (Px),           // 58-bit output: Primary data
        ...
        .A             (Ax),           // 34-bit input: A data
        .B             (Bx),           // 24-bit input: B data
        .C             (Cx),           // 58-bit input: C data
...
`elsif FPGA_FAMILY_7SERIES
    ...
    DSP48E1 #(
        ...
        .CLK           (clk),          // 1-bit input: Clock
        .INMODE        (INMODE),       // 5-bit input: INMODE control
        .OPMODE        (OPMODE[6:0]),  // 7-bit input: Operation mode
        // Data inputs: Data Ports
        .A             (A),            // 30-bit input: A data
        .B             (B),            // 18-bit input: B data
        .C             (C),            // 48-bit input: C data
        .CARRYIN       (0),            // 1-bit input: Carry-in
        .D             (D[24:0]),      // 25-bit input: D data
        // Reset/Clock Enable inputs: Reset/Clock Enable Inputs
        .CEA1          (0),            // 1-bit input: Clock enable for 1st stage AREG
        .CEA2          (PIPE_STAGE0),  // 1-bit input: Clock enable for 2nd stage AREG
        .CEAD          (0),            // 1-bit input: Clock enable for ADREG
    ...
    // Now assuming Ultrascale and Ultrascale+
`else
